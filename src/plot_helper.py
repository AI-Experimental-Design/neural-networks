...
def format_ax(ax, args):
    ...
    if args.xticks:
        items = [s.strip() for s in args.xticks.split(',') if s.strip()]
        if all(':' in it for it in items):
            ...
            locs = ax.get_xticks()
            n = min(len(locs), len(items))
            ax.set_xticks(locs[:n])
            ax.set_xticklabels(items[:n])

    if args.num_x_ticks and not args.xticks:
        ax.xaxis.set_major_locator(MaxNLocator(nbins=args.num_x_ticks))

    ax.tick_params(axis='x', rotation=args.xrotation)
...
def add_plot_args(parser):
    ...
    parser.add_argument('--num_x_ticks',
                        type=int,
                        help='Number of x ticks to show')
